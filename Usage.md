...
在默认情况下，sqlmap 会自动检测 Web 应用程序后端 DBMS的底层操作系统信息（当此信息是任何开关或选项的依赖时）。目前完全支持的操作系统是:

* Linux
* Windows

如果已知操作系统的名称,那么你就可以强制指定它,这样sqlmap就可以避免进行检测。

请注意，此选项不是强制性的，强烈推荐仅当你完全确定后端DBMS的底层操作系统时使用它。如果你不知道它，那么让sqlmap自动为你识别。

### 强制使用大数，以使失效值失效

开关: `--invalid-bignum`

当sqlmap需要使原始参数值失效时 (例如`id=13`),它会使用附属 (例如 `id=-13`)。使用这儿开关就可以强制使用大整数达到同样的目的。(例如`id=99999999`).

### 强制使用逻辑运算以使参数值无效
开关: `--invalid-logical`

当sqlmap需要使原参数无效效时(例如:`id=13`),它会使用负数(例如: `id=-13`)。使用这个开关可以强制使用布尔运算来达到同样的目的(例如 `id=13 AND 18=19`)。

### 强制使用随机字符串使参数值无效

开关: `--invalid-string`

当sqlmap需要使原参数无效效时(例如:`id=13`),它会使用负数(例如: `id=-13`)。使用这个开关可以强制使用随机字符串来达到同样的目的(例如 `id=akewmc`)。

### 关闭payload构造机制

开关:`--no-cast`

在返回结果时,sqlmap采用一种自带的机制，将所有条目都转换为字符串类型，并使用空格字符替换`NULL`。这样做是为了防止任何错误状态(例如使用字符串连接`NULL`值) 并且简化数据返回过程。然而,在一些反映的情况下(比如更低版本的MySQL DBMS)，由于数据检索本身存在问题（例如:返回了 None 值），需要通过这个开关关闭此机制。

### 关闭字符串转义机制

开关: `--no-escape`

当sqlmap需要使用payload中（单引号分隔）字符串时(例如 `SELECT 'foobar'`),这些数值都自动被转义(例如`SELECT CHAR(102)+CHAR(111)+CHAR(111)+CHAR(98)+CHAR(97)+CHAR(114)`)。其有两个原因：一是混淆payload，二是避免后端服务器上存在查询转义机制（例如:`magic_quotes` 和/或 `mysql_real_escape_string`）造成麻烦。用户可以使用这个开关来关闭它（例如缩短payload长度）。
